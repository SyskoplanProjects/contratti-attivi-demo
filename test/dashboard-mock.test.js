...
const mockCockpit = require('../app/contratti/webapp/model/mockCockpit');
const mockFornitori = require('../app/contratti/webapp/model/mockFornitori');

describe('mockCockpit', () => {
  test('tipologia segments sum to totaleContrattiAnno', () => {
    const fSum = mockCockpit.tipologia.reduce((n, s) => n + s.value, 0);
    expect(fSum).toBe(mockCockpit.totaleContrattiAnno);
  });
  test('survey segments sum to totaleContrattiAnno', () => {
    const fSum = mockCockpit.survey.reduce((n, s) => n + s.value, 0);
    expect(fSum).toBe(mockCockpit.totaleContrattiAnno);
  });
  test('trend has exactly 12 months summing attivati to totaleContrattiAnno', () => {
    expect(mockCockpit.trend.length).toBe(12);
    const fSum = mockCockpit.trend.reduce((n, m) => n + m.attivati, 0);
    expect(fSum).toBe(mockCockpit.totaleContrattiAnno);
  });
});

describe('mockFornitori', () => {
  test('has exactly 18 entries', () => {
    expect(mockFornitori.length).toBe(18);
  });
  test('includes the 3 fornitori that match real dev-DB intestatario values', () => {
    const aNomi = mockFornitori.map(f => f.nome);
    expect(aNomi).toEqual(expect.arrayContaining([
      'Banca Alpha S.p.A.', 'CloudTech Provider S.r.l.', 'Data Center Nord S.r.l.'
    ]));
  });
  test('every entry has the required numeric fields', () => {
    mockFornitori.forEach(f => {
      expect(typeof f.contrattiAttivi).toBe('number');
      expect(typeof f.contrattiPassivi).toBe('number');
      expect(typeof f.importoAttiviEuro).toBe('number');
      expect(typeof f.importoPassiviEuro).toBe('number');
      expect(typeof f.codiceSAP).toBe('string');
